 ...
 * @param {string} [expty='--'] - 如果枚举中寻找不到，返回的默认值，默认为 '--'。
 ...
 * @param {boolean} [isLimmediate=false] - “isLimmediate”参数决定该函数是在调用时立即执行还是等待指定的“delay”时间过去后才第一次执行该函数。
 ...
 * @param {boolean} [leading=true] - “leading”参数决定该函数是在调用时立即执行还是等待指定的“immediate”时间过去后才第一次执行该函数。
 * @param {boolean} [trailing=false] - “trailing”参数决定是否应在上次调用后立即调用节流函数，即使指定的时间间隔（“immediate”）尚未过去。
