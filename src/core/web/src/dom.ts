 ...
 * @beta
 * 该函数检查元素是否是节点。
 * @param {any} el - `el` 参数的类型是 `Element`，它表示 HTML DOM 中的一个元素。
 * @returns {Boolean} 一个布尔值。如果给定元素的 nodeType 为 1 (ELEMENT_NODE)、9 (DOCUMENT_NODE) 或 11
 ...
 * @returns {string} 所提供的 HTML 元素的字符串表示形式。
 * @example
 * ```JavaScript
 *    let div =document.createElement('div');
 ...
 * @param {Element | String} dom - "dom"参数可以是"Element"或"String"。它表示需要添加到"parentDom"的元素或 HTML 字符串。
 ...
 * @returns {Node | null} 创建的 div 元素的第一个子节点。
 * @example
 * ```JavaScript
 *    let str ='<div>123</div>'
 ...
 * @returns {Element | null} Element 对象或 null。
 * @example
 * ```JavaScript
 ...
 * @param {string} domName - `domName` 参数是一个字符串，表示您要选择的 DOM 元素的名称。
 * @returns {NodeList | null} NodeList 或 null。
 * @example
 * ```JavaScript
 ...
 * @param {Element} dom - `dom` 参数的类型为 `Node`，表示将执行类替换的 DOM 元素。
 ...
 * @returns {boolean} 一个布尔值，指示给定的 DOM 元素是否具有指定的类名。
 * @example
 * ```JavaScript
 *    let div =document.createElement('div');
